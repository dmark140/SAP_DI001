Public Class dmf_NodeCreator
    ...
    Sub iSaveNodes(ByVal cols As TreeNodeCollection)
        ...
        irecSetx.DoQuery(" SELECT top 1  Code  FROM [@DFM_TEST2]  A order by CONVERT(int,code) desc")
    ...
    Sub nodeUpdate()
        Try
            Dim irecSetx As SAPbobsCOM.Recordset
            irecSetx = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            irecSetx.DoQuery(" SELECT  U_void ,U_id   , code,Name    FROM [@DFM_TEST2]  A WHERE A.U_void = '1'  order by CONVERT(int,code)  ")
            Do While irecSetx.EoF = False

                Dim oUDT As SAPbobsCOM.UserTable
                oUDT = oCompany.UserTables.Item("DFM_TEST2") '' UDT NAME
                ...
                Else
                    MsgBox(irecSetx.Fields.Item("Name").Value & " Updated")
                End If
    ...
    Sub iInserNodes(ByVal iNodee As TreeNode)
        Try
            ...
            If retVal <> 0 Then
                oCompany.GetLastError(retVal, retStr)
                MsgBox("Error " & retVal & " " & retStr & " " & iNodeID_Inc)
            Else
                MsgBox("Record " & iNodeSplit(1) & " Added")
            End If
        Catch ex As Exception
